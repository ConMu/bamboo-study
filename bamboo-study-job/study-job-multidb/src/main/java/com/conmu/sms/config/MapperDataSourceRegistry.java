...
public class MapperDataSourceRegistry {
                ...
                if (bean != null) {
                    ...
                    for (Class<?> interfaceClass : interfaces) {
                        ...
                        if (className.startsWith(MAPPER_PACKAGE) && className.endsWith("Mapper")) {
                            // 根据Mapper类型分配不同的数据源
                            String dataSource = assignDataSource(className);
                            mapperDataSourceMap.put(className, dataSource);
                            discoveredMappers++;

                            String simpleName = className.substring(className.lastIndexOf(".") + 1);
                            System.out.println("🔍 [自动发现] " + simpleName + " -> " + dataSource);
                        }
                    }
                }
            }

            System.out.println("✅ [初始化完成] 自动发现并配置了 " + discoveredMappers + " 个Mapper");
    ...
    /**
     * 根据Mapper类名分配数据源
     * @param className Mapper全类名
     * @return 数据源标识
     */
    private String assignDataSource(String className) {
        // UserMapper 使用 ds1 (yunxin_recovery数据库)
        if (className.contains("UserMapper")) {
            return "ds1";
        }

        // PeopleMapper 使用 ds0 (test数据库) - 默认数据源
        if (className.contains("PeopleMapper")) {
            return "ds0";
        }
    ...
    private void fallbackToManualConfig() {
        ...
        mapperDataSourceMap.put("com.conmu.sms.dao.mapper.PeopleMapper", "ds0");  // test数据库
        mapperDataSourceMap.put("com.conmu.sms.dao.mapper.UserMapper", "ds1");    // yunxin_recovery数据库
        System.out.println("📋 [手动配置] PeopleMapper -> ds0, UserMapper -> ds1");
    }
